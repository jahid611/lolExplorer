...
export const getChampionData = async (championName: string) => {
  const response = await fetch(`https://ddragon.leagueoflegends.com/cdn/${DDRAGON_VERSION}/data/fr_FR/champion/${championName}.json`);
  const data = await response.json();
  return data.data[championName];
};

export const getItemData = async (itemId: number) => {
  const response = await fetch(`https://ddragon.leagueoflegends.com/cdn/${DDRAGON_VERSION}/data/fr_FR/item.json`);
  const data = await response.json();
